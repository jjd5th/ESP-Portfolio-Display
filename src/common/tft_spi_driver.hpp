...
namespace espidf
{
    ...
    struct tft_spi_driver
    {
        ...
        tft_spi_driver_result send_next_internal(const uint8_t *data, size_t size, bool queued, bool user, bool skip_batch_commit)
        {
            ...
            rr = m_spi_mgr.write(data, size, (void *)user, (queued) ? spi_transaction_type::queued : (use_polling) ? spi_transaction_type::polling
                                                                                                                   : spi_transaction_type::interrupt);
            if (spi_result::success != rr)
                return xlt_err(rr);
            return tft_spi_driver_result::success;
        }
    ...
    public:
        tft_spi_driver(tft_spi_driver_result *out_result = nullptr) : m_initialized(false),
